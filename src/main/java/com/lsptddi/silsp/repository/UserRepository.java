...
public interface UserRepository extends JpaRepository<User, Long> {
        ...
        @Query("SELECT new com.lsptddi.silsp.dto.AsesorListDto(u, COUNT(st)) " +
                        "FROM User u " +
                        "JOIN u.roles r " +
                        "LEFT JOIN SuratTugas st ON st.asesor = u " +
                        "WHERE r.name = 'Asesor' " +
                        "AND (:keyword IS NULL OR :keyword = '' OR LOWER(u.fullName) LIKE LOWER(CONCAT('%', :keyword, '%'))) "
                        +
                        "GROUP BY u")
        Page<AsesorListDto> findAsesorList(@Param("keyword") String keyword, Pageable pageable);
