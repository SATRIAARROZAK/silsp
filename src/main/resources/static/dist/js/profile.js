$(document).ready(function () {
  ...
  // ==========================================
  // 1. PREVIEW FOTO REALTIME (Avatar)
  // ==========================================
  $("#avatarInput").change(function () {
    const file = this.files[0];
    if (file) {
      ...
      reader.onload = function (event) {
        $("#avatarPreview").attr("src", event.target.result);
        // Update juga preview di Navbar/Sidebar jika ID-nya ada
        $(".user-image, .img-circle").attr("src", event.target.result);
      };
      reader.readAsDataURL(file);
    }
  });
  ...
  // ==========================================
  // 3. LOGIKA PASSWORD (REALTIME & MODAL)
  // ==========================================

  // Cek Realtime Password Lama
  $("#m_current").on("blur", function () {
    var pass = $(this).val();
    if (pass) {
      $.post(
        ...
        function (valid) {
          if (!valid) {
            $("#m_current").addClass("is-invalid");
            // Tambahkan pesan error manual jika belum ada
            if ($("#m_current").next(".invalid-feedback").length === 0) {
              $("#m_current").after(
                '<div class="invalid-feedback">Password saat ini salah!</div>'
              );
            }
          } else {
            $("#m_current").removeClass("is-invalid").addClass("is-valid");
          }
        }
      );
    }
  ...
  $("#btnApplyPass").click(function () {
    ...
    if (!current || !newVal || !confirm) {
      Swal.fire("Peringatan", "Lengkapi semua kolom password", "warning");
      return;
    }
    if ($("#m_current").hasClass("is-invalid")) {
      Swal.fire("Error", "Password lama masih salah", "error");
      return;
    }
    if (newVal.length < 8) {
      Swal.fire("Peringatan", "Password baru min 8 karakter", "warning");
      return;
    }
    if (newVal !== confirm) {
    ...
    // Set ke Hidden Input Form Utama
    ...
    Swal.fire(
      "Berhasil",
      "Password telah di-set. Klik Simpan Perubahan untuk memproses.",
      "success"
    );
  });
...
// $(document).ready(function () {
//   // ==========================================
//   // 4. LOGIKA TANDA TANGAN (CANVAS + UPLOAD)
//   // ==========================================
//   var canvas = document.getElementById("signature-canvas");
//   var signaturePad = new SignaturePad(canvas, {
//     backgroundColor: "rgba(255, 255, 255, 0)",
...
//     canvas.getContext("2d").scale(ratio, ratio);
//   }
//   $("#modalSignature").on("shown.bs.modal", resizeCanvas);
//   $("#btnClearSig").click(function () {
//     signaturePad.clear();
//   });

//   // FIX FITUR UPLOAD GAMBAR TTD
//   $("#btnUpload").click(function () {
//     $("#uploadSigFile").click();
//   });

//   $("#uploadSigFile").change(function (e) {
//     var file = e.target.files[0];
//     if (!file) return;

//     var reader = new FileReader();
//     reader.onload = function (event) {
//       // Load gambar ke canvas SignaturePad
//       signaturePad.fromDataURL(event.target.result, {
//         ratio: 1,
//         width: 300,
//         height: 150,
...
//   // ==========================================
//   $("#formProfile").validate({
//     ignore: [], // Jangan ignore hidden
//     rules: {
//       fullName: { required: true },
//       email: { required: true, email: true },
//       noTelp: { required: true, number: true },
//       // Tambahkan validasi lain sesuai role
//     },
//     messages: {
//       fullName: "Nama harus diisi",
//       email: "Email tidak valid",
//       noTelp: "No Telp harus angka",
//     },
//     errorElement: "span",
//     errorPlacement: function (error, element) {
//       error.addClass("invalid-feedback");
