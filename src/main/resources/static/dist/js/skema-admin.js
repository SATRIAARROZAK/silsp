...
$(document).ready(function () {
  ...
  if ($("#form-tambah-skema").length) {
    ...
    $(".next-tab").on("click", function () {
      const currentTab = $(this).closest(".tab-pane");
      const res = validateTab(currentTab);

      if (res.isValid) {
        const targetTabId = $(this).data("target-tab");
        $("#" + targetTabId).tab("show");
      } else {
        Toast.fire({ icon: "error", title: "Lengkapi data wajib!" });
        if (res.firstInvalidElement) {
          // Cek jika itu summernote
          if (res.firstInvalidElement.hasClass("summernote-persyaratan")) {
            res.firstInvalidElement.summernote("focus");
          } else {
            res.firstInvalidElement.focus();
          }
        }
      }
    });
